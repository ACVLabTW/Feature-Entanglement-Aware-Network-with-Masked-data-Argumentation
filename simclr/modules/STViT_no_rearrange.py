    ...
    def stforward(self, X, return_seq=True):
        ...
        if len(X['img'])==self.FRR:
            for k in range(len(X['img'])):
                ...
                # print('conv feature', x.shape) # conv feature torch.Size([8, 1024, 5, 5])
                x = self.reshape(x)
                # print('reshape', x.shape) # reshape torch.Size([8, 1, 25600])
                module_outputs.append(x)

            # print('module_outputs', len(module_outputs), module_outputs[0].shape) # module_outputs 16 torch.Size([8, 1, 25600])
            
            # change to covariance matrix here~~~
            feat = torch.cat(module_outputs, 1) #(hw*FRR) x c
            # feat = torch.bmm(feat.transpose(1,2),feat) # change to covariance matrix
            
#             sts()
            # print('before svt',feat.shape) # before svt torch.Size([8, 16, 25600])
            if self.heads != 0:
                feat, feat_seq = self.spatial_transformer(feat,None,True)
            elif self.heads == 0:
                feat, feat_seq = self.spatial_transformer(feat)
            # print('feat',feat.shape, 'feat_seq', feat_seq.shape) # feat torch.Size([8, 25600]) feat_seq torch.Size([8, 17, 25600])
            ...
            if self.masked_reconstruction: # error here, so let self.mask_recostruction=False temporally
                rec_seq = []
                feature_seq = []
                feat_seq = feat_seq[:, 1:]
                ...
                for z in range(self.FRR):
                    ...
                    if self.use_mask:
                        ...
                        f = feat_seq[:, ind:ind+self.patch_size]
                    # print('f',f.shape) # [8, 25, 1024] --> [8, 16, 25600]
                    #         ('b c h w -> b (h w) c') --> ('b c h w -> b () (h w c)')
                  # ('b (h w) c -> b c h w', h=5, w=5) --> ('b f (h w c) -> b c h w', c=1024, h=5, w=5)
                                     # [8, 1024, 5, 5] --> []
                    f = self.recover(f)
