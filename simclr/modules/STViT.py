...
#             sts()
            ...
            if self.masked_reconstruction:
                ...
                for z in range(self.FRR):
                    ...
                    # print('recover f',f.shape)
                    rec_seq.append(self.gen(f))
                # print('gan rec_seq',len(rec_seq), rec_seq[0].shape)
                rec_seq = torch.stack(rec_seq).permute(1,0,2,3,4)
                # print('rec_seq', rec_seq.shape)
            else:
                rec_seq = []
    ...
    def forward(self, X, forward_mode='train'):
        X=self.stforward(X)
        # print('forward X', X[0].shape, X[1].shape)
        cls=self.cls(X[0])
        # print('cls', cls.shape)
...
class SingleViT2_one_neuron(nn.Module):
    """
    We opt for simplicity and adopt the commonly used ResNet (He et al., 2016) to obtain hi = f(x ̃i) = ResNet(x ̃i) where hi ∈ Rd is the output after the average pooling layer.
    """

    def __init__(self, device, args, mode='train'):
        super(SingleViT2_one_neuron, self).__init__()
        self.mode=mode
        self.FREQ=args.FREQ
        self.backbone=args.backbone
        self.device = device
        ...
        self.cls = nn.Sequential( nn.Linear(args.n_features, 1),
                                  nn.Sigmoid() )
