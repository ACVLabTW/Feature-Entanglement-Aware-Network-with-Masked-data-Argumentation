...
#         model.load_state_dict()
    ...
    for epoch in range(args.current_epoch, args.epochs):
        ...
        if epoch% 10==0:
            ...
            with torch.no_grad():
                ...
                if accs>=best_accs:
                    best_accs = accs
                    save_model(args, model, optimizer, filename='best_epoch{}.pth'.format(epoch), dict_only=True)


                if args.local_rank==0:
                    print('-'*80)
                    print('Validation accuracy:', np.mean(np.array(accset)) )
                    print('accs, f1, re, pr, auc:', accs, f1, re, pr, auc )
    ...
    ## end training
    save_model(args, model, optimizer, filename='last.pth', dict_only=True)
