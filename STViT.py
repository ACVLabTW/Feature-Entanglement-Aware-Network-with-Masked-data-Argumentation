        ...
        if len(X['img'])==self.FRR:
            for k in range(len(X['img'])):
                if 'efficientnet' in self.backbone:
                    ...
                    x = self.encoder.extract_features(X['img_mask'][k].to(self.device, non_blocking=True))

                elif self.encoder is not None:
                    # x = self.encoder(X['img'][k].to(self.device, non_blocking=True))
                    x = self.encoder(X['img_mask'][k].to(self.device, non_blocking=True))

                else:
                    # x = X['img'][k].float().to(self.device, non_blocking=True)
                    x = X['img_mask'][k].float().to(self.device, non_blocking=True)
...
#             sts()
            feat, feat_seq = self.spatial_transformer(feat, return_seq=True)
